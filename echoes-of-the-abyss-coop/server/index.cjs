...
function createRelay({origins=process.env.ALLOWED_ORIGINS||'https://mijun-byte.github.io,http://localhost:8080,http://127.0.0.1:8080',maxRooms=100}={}){
 ...
 const server=http.createServer((req,res)=>{res.setHeader('Content-Type','application/json');res.writeHead(req.url==='/health'?200:404);res.end(JSON.stringify(req.url==='/health'?{ok:true,protocol:2}:{error:'Not found'}));});
 ...
 const lobbyState=room=>({type:'lobby_state',players:['host','guest'].map(role=>{const peer=room.peers.find(p=>p.role===role);return peer?{profile:peer.profile,ready:!!peer.ready}:{profile:null,ready:false};})});
 const broadcastLobby=room=>{const state=lobbyState(room);for(const peer of room.peers)send(peer,state);};
 ...
 wss.on('connection',s=>{
  ...
  s.on('message',(raw,binary)=>{
   ...
   if(m.type==='create'||m.type==='join'){
    ...
    if(m.protocol!==2)return fail(s,'Versions incompatibles : actualisez tous les deux le jeu.');
    ...
    if(m.type==='create'){
     ...
     s.code=code;s.role='host';s.profile=m.profile;s.ready=true;const room={peers:[s],started:false};rooms.set(code,room);send(s,{type:'room',role:'host',code});broadcastLobby(room);
    }else{
     ...
     s.code=code;s.role='guest';s.profile=m.profile;s.ready=false;room.peers.push(s);send(s,{type:'room',role:'guest',code});broadcastLobby(room);for(const peer of room.peers)send(peer,{type:'ready',profiles:room.peers.map(p=>p.profile)});
    }return;
   }
   const room=rooms.get(s.code);if(!room)return;const other=room.peers.find(p=>p!==s);
   if(m.type==='lobby_update'&&!room.started){
    if(m.profile&&JSON.stringify(m.profile).length<=4096)s.profile=m.profile;
    if(s.role==='guest'&&typeof m.ready==='boolean')s.ready=m.ready;
    broadcastLobby(room);return;
   }
   if(m.type==='start'&&s.role==='host'&&room.peers.length===2&&!room.started){if(!room.peers.every(p=>p.ready&&p.profile))return fail(s,'Attends que ton partenaire soit prêt avant de lancer.');room.started=true;for(const peer of room.peers)send(peer,{type:'start'});return;}
