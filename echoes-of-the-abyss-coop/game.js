...
function coopMenu(message=''){
 ...
 $('#coopServer').value=prefs.url||window.ABYSS_COOP_SERVER||'';
 $('.coop-note').textContent='Ton pseudo et tes préférences sont mémorisés dans ce navigateur. Tu peux changer ton pseudo ci-dessus. Une déconnexion pendant une expédition termine la partie coop.';
 const reload=document.createElement('button');reload.id='coopReload';reload.className='button-quiet';reload.textContent='Actualiser le jeu · conserver ma progression';reload.onclick=()=>{const url=new URL(location.href);url.searchParams.set('refresh',Date.now());location.replace(url.href);};$('#modalCard').appendChild(reload);
 const invitation=new URLSearchParams(location.search).get('join');if(invitation&&/^[0-9A-F]{10}$/i.test(invitation))$('#coopCode').value=invitation.toUpperCase();
...
function renderCoopLobbyPlayers(c){
 const profiles=c.lobbyPlayers?.map(p=>p.profile)||(c.profiles?[...c.profiles]:[null,null]);profiles[c.local]??=c.localProfile;
...
function coopLobby(){
 ...
 showModal(`<div class="coop-lobby"><header class="lobby-header"><div><div class="eyebrow">VOTRE GROUPE · SALON PRIVÉ</div><h2>Le seuil de l’Abîme</h2><p>Préparez vos aventuriers. Descendez ensemble.</p></div><div class="coop-lobby-code"><span>CODE D’INVITATION</span><strong class="coop-code">${c.code}</strong><button id="coopCopy">Copier l’invitation</button></div></header><div class="coop-lobby-details"><span>Joueurs <b>${count} / 2</b></span><span>Mode <b>Coop privée</b></span><span>Progression <b>Infini · Étage 1</b></span></div><div class="coop-lobby-grid">${coopLobbyPlayers(c)}</div><p class="coop-lobby-status" role="status">${status}</p><div class="coop-lobby-actions"><button id="coopCustomize">Personnaliser mon personnage</button>${c.role==='guest'?`<button id="coopReady">${c.ready?'Annuler Prêt':'Prêt'}</button>`:''}${c.role==='host'?`<button id="coopStart" ${canStart?'':'disabled'}>Lancer l’expédition à deux</button>`:''}<button id="coopLeave" class="button-quiet">Quitter le salon</button></div></div>`);renderCoopLobbyPlayers(c);
 $('#coopCopy').onclick=async()=>{const button=$('#coopCopy');try{const url=new URL(location.href);url.search='';url.hash='';url.searchParams.set('join',c.code);await navigator.clipboard.writeText(url.href);if(button.isConnected)button.textContent='Invitation copiée';}catch{if(button.isConnected)button.textContent='Sélectionne le code ci-dessus pour le copier';}};
