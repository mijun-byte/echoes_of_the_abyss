...
function awardGold(amount){const n=Math.round(amount*game.player.goldMul);game.player.gold+=n;game.stats.gold+=n;}
...
class Projectile{
 constructor(x,y,a,speed,damage,owner,c="#ddd",r=5,source=null){this.source=source;Object.assign(this,{x,y,px:x,py:y,a,speed,damage,owner,c,r,life:2.2,vx:Math.cos(a)*speed,vy:Math.sin(a)*speed})}
 update(dt){
...
class Player{
 constructor(){
  const meta=loadMeta(); this.x=W/2;this.y=H/2;this.r=15;this.maxHp=100+(meta.hp||0)*5;this.hp=this.maxHp;this.speed=235;
 ...
 heal(v){this.hp=Math.min(this.maxHp,this.hp+v*this.healMul)}
 hit(d){
  if(this.ward>0||this.inv>0||this.dashing>0||Math.random()<this.dodge)return;
  ...
  audio("hurt"); if(this.hp<=0) game.gameOver();
 }
 ...
 draw(){
  ...
  drawCreature("player",0,0,44,this.walkT,"#65e0d1");
...
function attackSource(p){return {weapon:p.weapon.type,life:p.weapon.extra==='life'?.03:0,burn:p.element?12*p.damageMul:0};}
...
class Enemy{
 ...
 die(){
  if(this.dead)return;this.dead=true;game.player.kills++;game.stats.kills++;if(this.lastHitWeapon){game.stats.weaponKills=game.stats.weaponKills||{};game.stats.weaponKills[this.lastHitWeapon]=(game.stats.weaponKills[this.lastHitWeapon]||0)+1;}checkChallenges();game.player.gainXp(this.elite?32:14);burst(this.x,this.y,this.elite?"#bf67ff":"#8e3e43",18);audio("death");
...
class Boss extends Enemy{
 ...
 resolveAttack(){
  ...
  if(action.type==='charge'){
   ...
   if(pointSegmentDistance(p,from,this)<this.r+p.r+8)p.hit(this.damage*1.25);
  ...
  if(action.type==='shockwave'){if(dist(action,p)<action.r+p.r)p.hit(this.damage*1.3);spellEffect({type:'earth',x:action.x,y:action.y,r:action.r,color:'#f3b16f',life:.65});}
...
class Game{
 ...
 handleInteraction(){
  ...
  const d=[...this.drops,...(this.room.shop||[])].filter(d=>!d.taken&&!d.bought&&dist(this.player,d)<(d.price?60:55)).sort((a,b)=>dist(this.player,a)-dist(this.player,b))[0];
 ...
 draw(){
  ...
  for(const e of this.enemies){e.draw();drawStatuses(e);}for(const p of this.projectiles)p.draw();this.player.draw();
...
function nearestDrop(range){return [...game.drops,...(game.room.shop||[])].filter(d=>!d.taken&&!d.bought&&d.type!=='exit'&&dist(game.player,d)<range).sort((a,b)=>dist(game.player,a)-dist(game.player,b))[0];}
function drawDrop(d){
...
$("#mainMenu").addEventListener("click",e=>{let a=e.target.dataset.action;if(a==="new")startGame();if(a==="continue")continueRun();if(a==="options")options();if(a==="help")help();if(a==="meta")metaMenu();if(a==="shop")bankShop();if(a==="roulette")rouletteMenu();if(a==="customize")customization();if(a==="challenges")challengesMenu()});
function loop(t){let dt=Math.min(.033,(t-last)/1000);last=t;if(game){game.update(dt);game.draw()}requestAnimationFrame(loop)}requestAnimationFrame(loop);mainMenu();
const SOULS=[
